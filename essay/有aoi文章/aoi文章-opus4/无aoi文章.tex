...
\section{引言}
随着物联网(IoT)和5G/6G通信技术的快速发展，移动设备产生的数据量呈指数级增长。移动边缘计算(MEC)通过将计算资源部署在网络边缘，为延迟敏感型应用提供了有效解决方案\cite{b1}。在MEC系统中，任务卸载决策直接影响系统性能，需要在能耗和延迟之间进行权衡。
...
本文的主要贡献如下：
\begin{itemize}
    \item 建立了综合考虑能耗和延迟的双目标优化模型，采用30\%-70\%权重设置，并证明其NP-hard特性
    \item 提出了基于双阶段集成的TLBO-HHO融合算法，通过自适应概率机制(0.6→0.1)实现探索与开发的动态平衡
    \item 在包含20设备、4边缘服务器、2云服务器和40任务的场景中，验证了算法的有效性
\end{itemize}
\section{相关工作}
\subsection{元启发式优化算法}
Rao\cite{b5}提出的TLBO算法模拟教学过程，具有参数少、收敛稳定的优点。Heidari等\cite{b6}提出的HHO算法通过模拟鹰群狩猎行为，在处理多峰函数时成功率达85-95\%。最新的混合策略包括双阶段集成\cite{b7}和参数协调机制\cite{b8}，但TLBO-HHO在MEC任务卸载中的应用仍然稀缺。
\subsection{MEC任务卸载优化}
任务卸载优化方法主要包括：(1)数学优化方法，如拉格朗日对偶分解\cite{b9}和Lyapunov优化\cite{b10}，理论基础扎实但难以处理非凸问题；(2)机器学习方法，如多智能体强化学习\cite{b11,b13}，适应性强但需要大量训练数据；(3)元启发式方法，在处理NP-hard问题时表现良好\cite{b12,b17}，能够实现高达56\%的能耗降低。
\subsection{研究差距}
现有研究的不足包括：(1)缺乏深度融合机制，特别是TLBO-HHO的直接混合实现；(2)75\%的研究局限于小规模场景；(3)仅25\%进行了真实测试床验证；(4)缺乏统一的基准测试集。本文旨在通过创新的双阶段集成机制填补这些空白。
\section{系统模型与问题陈述}
\subsection{网络架构与场景描述}\label{AA}
考虑一个典型的三层MEC网络架构，如图1所示。
\begin{figure}[htbp]
    \centering
    ...
    \label{fig1}
\end{figure}
该架构反映了实际部署中的层次化计算资源分布，与商业MEC平台(如AWS Wavelength、Azure Edge Zones)的部署模式一致：
系统包含以下组件：
\begin{itemize}
...
\end{equation}
其中，${f}_{i,j}^{d}$、${f}_{i,j}^{e}$、${f}_{i,j}^{c}$ 分别表示分配给任务 i 的CPU频率，受到DVFS(动态电压频率调节)技术的约束。
\subsubsection{传输延迟}
...
\subsection{优化问题建模}
\subsubsection{决策变量}
定义二进制决策变量 $\mathbf{x} = \{x_i^{local}, x_i^{edge}, x_i^{cloud}\}_{i \in {T}}$，其中：
\begin{itemize}
    \item ${x}_{i}^{local}$ 表示任务 i 在本地设备执行
    \item ${x}_{i}^{edge}$ 表示任务 i 卸载到边缘服务器
    \item ${x}_{i}^{cloud}$ 表示任务 i 卸载到云服务器
\end{itemize}
连续决策变量 ${f} = \{f_{i,j}\}_{i \in {T}, j \in {D} \cup {E} \cup {C}}$ 表示分配的CPU频率。
\subsubsection{目标函数}
基于实际应用需求和最新研究趋势，采用加权和方法将双目标转化为单目标：
\begin{equation}
\min_{\mathbf{x}, \mathbf{f}} F = \sum_{i \in {T}} \left( w^E \cdot \frac{E_i}{E^{norm}} + w^T \cdot \frac{T_i}{T^{norm}} \right)
\end{equation}
其中：
\begin{itemize}
...
\medskip
此外，完整模型中既含二进制变量 $\mathbf{x}$，又含连续变量 $\mathbf{f}$，且目标函数包含非线性项（如 $f_i^\alpha$），因此属于混合整数非线性规划（MINLP），求解难度进一步加大。
...
\end{itemize}
如图2所示，任务类型的分布反映了实际MEC应用场景的多样性需求。
\begin{figure}[htbp]
    \centering
    \centerline{\includegraphics[width=0.9\linewidth]{task_characteristics.png}}
    \caption{任务类型分布及其特征.}
    \label{fig2}
\end{figure}
...
图3展示了四种算法在200次迭代中的收敛曲线对比，包含了10次独立运行的平均值和标准差阴影区域。
\begin{figure}[htbp]
    \centering
    \centerline{\includegraphics[width=0.9\linewidth]{convergence_curves.png}}
    \caption{不同算法的收敛曲线对比.}
    \label{fig3}
\end{figure}
从收敛曲线可以清晰观察到：
...
\subsection{性能对比分析}
    ...
    图4展示了四种算法的详细性能对比，包括能耗、延迟、违规率和适应度值的分布。
    \begin{figure}[htbp]
        \centering
        \centerline{\includegraphics[width=0.9\linewidth]{device_capabilities.png}}
        \caption{不同算法的收敛曲线对比.}
        \label{fig4}
    \end{figure}
    从图3可以看出：
...
\begin{figure}[htbp]
    \centering
    \centerline{\includegraphics[width=0.9\linewidth]{performance_metrics.png}}
    \caption{总能耗与总延迟对比.}
    \label{fig5}
\end{figure}
关键发现：
...
\bibitem{b4} Guo S, Xiao B, Yang Y, et al. Energy-efficient dynamic offloading and resource scheduling in mobile cloud computing[C]//IEEE INFOCOM 2016-The 35th Annual IEEE International Conference on Computer Communications. IEEE, 2016: 1-9.
...
\bibitem{b10} Bi S, Zhang Y J. Computation rate maximization for wireless powered mobile-edge computing with binary computation offloading[J]. IEEE Transactions on Wireless Communications, 2018, 17(6): 4177-4190.
\bibitem{b11} Gao Z, Yang L, Dai Y. Fast adaptive task offloading and resource allocation in large-scale MEC systems via multiagent graph reinforcement learning[J]. IEEE Internet of Things Journal, 2023, 11(1): 758-776.
\bibitem{b12} Adhikari M, Srirama S N, Amgoth T. Application offloading strategy for hierarchical fog environment through swarm optimization[J]. IEEE Internet of Things Journal, 2019, 7(5): 4317-4328.
